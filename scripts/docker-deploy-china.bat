...
echo %info_prefix% 构建Docker镜像（使用中国镜像源）...

REM 构建后端镜像
echo %info_prefix% 构建后端镜像...
docker build -f docker/backend/Dockerfile.china -t ai-sound-backend .
if errorlevel 1 (
...
docker build -f docker/frontend/Dockerfile.china -t ai-sound-frontend .
if errorlevel 1 (
...
:pull_images
echo %info_prefix% 拉取基础镜像（使用中国镜像源）...
...
docker tag registry.cn-hangzhou.aliyuncs.com/library/redis:alpine redis:alpine
...
:show_access_info
echo.
echo 🎉 AI-Sound Docker部署完成（中国镜像源版本）！
echo.
echo 访问地址：
...
:help
echo 用法: %0 [check^|build^|pull^|infra^|app^|full]
echo   check - 检查前置条件
echo   build - 构建Docker镜像（中国镜像源）
echo   pull  - 拉取基础镜像（中国镜像源）
echo   infra - 启动基础设施
echo   app   - 启动应用服务
echo   full  - 完整部署（默认）
goto :EOF
