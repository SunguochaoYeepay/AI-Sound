...
try:
    from memory_manager import memory_context, clear_memory, check_memory_pressure, TorchOptimizer
except ImportError:
    print("Warning: Memory manager not available, using basic memory management")
    memory_context = lambda **kwargs: contextlib.nullcontext()
    clear_memory = lambda aggressive=False: torch.cuda.empty_cache() if torch.cuda.is_available() else None
    check_memory_pressure = lambda: False
    import contextlib
...
class MegaTTS3DiTInfer():
    ...
    def preprocess(self, audio_bytes, latent_file=None, topk_dur=1, **kwargs):
        ...
        if check_memory_pressure():
            print("Warning: High memory pressure detected during preprocessing, performing aggressive cleanup")
        ...
        with memory_context(clear_after=True, log_usage=True):
    ...
    def forward(self, resource_context, input_text, time_step, p_w, t_w, dur_disturb=0.1, dur_alpha=1.0, **kwargs):
        ...
        # 显存管理：推理前检查内存压力
        if check_memory_pressure():
            print("Warning: High memory pressure detected, performing aggressive cleanup")
            clear_memory(aggressive=True)
