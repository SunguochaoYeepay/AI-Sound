<template>
  <div class="login-container">
      ...
      >
        ...
        <a-form-item>
          <div class="login-options">
            <a-checkbox v-model:checked="loginData.remember_me"> 记住我 </a-checkbox>
        ...
        <div class="login-footer">
          <span>还没有账号？</span>
          <a-button type="link" @click="showRegisterModal = true"> 立即注册 </a-button>
        </div>
      </a-form>
    </div>

    <!-- 注册模态框 -->
    <a-modal v-model:open="showRegisterModal" title="用户注册" :footer="null" width="480px">
      <a-form
        ref="registerForm"
        :model="registerData"
        :rules="registerRules"
      ...
      >
        ...
        <a-form-item name="full_name" label="真实姓名">
          <a-input v-model:value="registerData.full_name" placeholder="请输入真实姓名（可选）" />
        </a-form-item>
    ...
    <!-- 忘记密码模态框 -->
    <a-modal v-model:open="showForgotPassword" title="重置密码" :footer="null">
      <a-result
        status="info"
        title="密码重置"
      ...
      >
        <template #extra>
          <a-button type="primary" @click="showForgotPassword = false"> 我知道了 </a-button>
        </template>
      </a-result>
    </a-modal>
  </div>
...
<script setup>
  ...
  import { UserOutlined, LockOutlined, SoundOutlined } from '@ant-design/icons-vue'
...
<style scoped>
  ...
  [data-theme='dark'] :deep(.ant-input-affix-wrapper:focus, .ant-input-affix-wrapper-focused) {
    border-color: var(--primary-color) !important;
    box-shadow: 0 0 0 2px rgba(var(--primary-color-rgb), 0.2) !important;
  }
