...
const allPermissions = ref([
  { name: 'tts:create', display_name: 'TTS服务创建', description: '创建TTS转换任务', group: 'tts' },
  { name: 'tts:delete', display_name: 'TTS服务删除', description: '删除TTS转换记录', group: 'tts' },
  ...
  { name: 'project:create', display_name: '项目创建', description: '创建新项目', group: 'project' },
  { name: 'project:edit', display_name: '项目编辑', description: '编辑项目信息', group: 'project' },
  { name: 'project:delete', display_name: '项目删除', description: '删除项目', group: 'project' },
  { name: 'system:config', display_name: '系统配置', description: '系统配置管理', group: 'system' },
  { name: 'system:monitor', display_name: '系统监控', description: '系统监控查看', group: 'system' }
])
...
const toggleGroup = (group) => {
  ...
  if (isSelected) {
    // 取消选择该组的所有权限
    roleForm.permissions = roleForm.permissions.filter(p => 
      !group.permissions.some(gp => gp.name === p)
    )
  } else {
    // 选择该组的所有权限
...
const toggleGroupPermission = (group) => {
  ...
  if (isSelected) {
    selectedPermissions.value = selectedPermissions.value.filter(p => 
      !group.permissions.some(gp => gp.name === p)
    )
  } else {
    const groupPermissions = group.permissions.map(p => p.name)
