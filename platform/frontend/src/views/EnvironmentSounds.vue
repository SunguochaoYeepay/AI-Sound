        ...
        <div class="sounds-grid">
          ...
          >
            ...
            <div v-if="sound.generation_status === 'failed'" class="error-message">
              <a-alert type="error" :message="sound.error_message || '生成失败'" banner />
            </div>
          </div>
        </div>

        <!-- 空状态 -->
        <a-empty v-if="!loading && sounds && sounds.length === 0" description="暂无环境音">
          <a-button type="primary" @click="showGenerateModal = true"> 立即生成 </a-button>
        </a-empty>
      ...
      <div class="smart-analysis-content">
          ...
          <div v-if="analysisResult && !analyzing" class="analysis-result">
            ...
            <a-card title="分析摘要" style="margin-bottom: 16px">
              <a-descriptions :column="2" size="small">
                <a-descriptions-item label="总轨道数">{{
                  analysisResult.total_tracks || analysisResult.total_scenes || 0
                }}</a-descriptions-item>
                <a-descriptions-item label="分析模式">{{
                  analysisResult.llm_provider || '章节分析'
                }}</a-descriptions-item>
                <a-descriptions-item label="总时长"
                  >{{ analysisResult.total_duration || 0 }}秒</a-descriptions-item
                >
                <a-descriptions-item label="章节数">{{
                  analysisResult.chapters_analyzed || 1
                }}</a-descriptions-item>
              </a-descriptions>

              <div v-if="analysisResult.narrative_analysis" style="margin-top: 16px">
                <a-tag color="blue">{{
                  analysisResult.narrative_analysis.genre || '未知体裁'
                }}</a-tag>
                <a-tag color="green">{{
                  analysisResult.narrative_analysis.pace || '中等节奏'
                }}</a-tag>
            ...
            <a-card title="环境音轨道">
              <template v-if="analysisResult.chapters && analysisResult.chapters.length > 0">
                  ...
                  <div class="tracks-list">
                    ...
                    >
                      <div class="track-header">
                        <h5>轨道 {{ index + 1 }}</h5>
                        <a-tag :color="getIntensityColor(track.intensity_level)">{{
                          track.intensity_level || '中等'
                        }}</a-tag>
                      </div>
                      <div class="track-details">
                        <a-space>
                          <a-tag
                            >🕐 {{ track.start_time }}s -
                            {{ track.end_time || track.start_time + track.duration }}s</a-tag
                          >
                      ...
                      <div
                        v-if="track.environment_keywords && track.environment_keywords.length > 0"
                        class="track-keywords"
                      >
                ...
                <div class="total-stats">
                  <a-space>
                    <a-statistic
                      title="总章节数"
                      :value="analysisResult.chapters_analyzed || analysisResult.chapters.length"
                    />
                    <a-statistic title="总轨道数" :value="analysisResult.total_tracks" />
                    <a-statistic
                      title="总时长"
                      :value="analysisResult.total_duration"
                      suffix="秒"
                    />
                  </a-space>
                </div>
              ...
              <template
                v-else-if="
                  analysisResult.environment_tracks && analysisResult.environment_tracks.length > 0
                "
              >
                ...
                <div class="tracks-list">
                  ...
                  >
                    ...
                    <!-- 显示匹配结果 -->
                    <div
                      v-if="track.has_match && track.best_match"
                      class="track-match-info"
                ...
                <div v-if="analysisResult.matching_summary" class="matching-summary">
                  ...
                  <a-row :gutter="16">
                    <a-col :span="6">
                      <a-statistic
                        title="总轨道数"
                        :value="analysisResult.matching_summary.total_tracks"
                      />
                    </a-col>
                    <a-col :span="6">
                      <a-statistic
                        title="已匹配"
                        :value="analysisResult.matching_summary.matched_tracks"
                      />
                    </a-col>
                    <a-col :span="6">
                      <a-statistic
                        title="需要生成"
                        :value="analysisResult.matching_summary.need_generation_tracks"
                      />
                    </a-col>
                    <a-col :span="6">
                      <a-statistic
                        title="匹配率"
                        :value="analysisResult.matching_summary.match_rate"
                        suffix="%"
                      />
                    </a-col>
                  </a-row>
        ...
        <div v-if="analysisStep === 2" class="analysis-step">
          ...
          <div v-if="smartPrompts && !generatingPrompts" class="smart-prompts-result">
            ...
            <!-- 音景推荐 -->
            <a-card
              v-if="smartPrompts.soundscape_recommendation"
              title="整体音景设计"
            ...
            >
              <a-descriptions :column="2" size="small">
                <a-descriptions-item label="主要元素">
                  <a-tag
                    v-for="element in smartPrompts.soundscape_recommendation.primary_elements"
                    :key="element"
                    color="blue"
                  >
                    {{ element }}
                ...
                <a-descriptions-item label="次要元素">
                  <a-tag
                    v-for="element in smartPrompts.soundscape_recommendation.secondary_elements"
                    :key="element"
                    color="green"
                  >
                    {{ element }}
                ...
                <a-descriptions-item label="建议总时长"
                  >{{
                    smartPrompts.soundscape_recommendation.overall_duration
                  }}秒</a-descriptions-item
                >
                <a-descriptions-item label="环境层次">{{
                  smartPrompts.soundscape_recommendation.ambient_layers?.join(', ')
                }}</a-descriptions-item>
              </a-descriptions>
            </a-card>
                ...
                >
                  <div class="prompt-header">
                    ...
                    <a-space>
                      ...
                      <a-tag :color="getPriorityColor(prompt.priority)"
                        >优先级 {{ prompt.priority }}</a-tag
                      >
                  ...
                  <div
                    v-if="prompt.dynamic_elements && prompt.dynamic_elements.length > 0"
                    class="prompt-features"
                  >
                    <strong>动态元素:</strong>
                    <a-tag
        ...
        <div v-if="analysisStep === 3" class="analysis-step">
          ...
          <a-card>
            ...
            <div class="step-actions" style="margin-top: 16px">
              <a-space>
                <a-button
                  v-if="batchProgress.status !== 'active'"
                  @click="showSmartAnalysisModal = false"
                >
                  关闭
                </a-button>
                <a-button
                  v-if="batchProgress.status === 'active'"
                  @click="cancelBatchGeneration"
                  danger
                >
                  取消生成
                </a-button>
...
<script setup>
  ...
  const loadInitialData = async () => {
    await Promise.all([loadCategories(), loadTags(), loadPresets(), loadStats(), loadSounds()])
  }
  ...
  const loadBooks = async () => {
    try {
      ...
      books.value =
        response.data.success && response.data.data ? response.data.data : response.data.data || []
    } catch (error) {
  ...
  const loadBookChapters = async () => {
    ...
    try {
      ...
      // 过滤出已完成智能分析的章节
      analyzedChapters.value = allChapters.filter(
        (chapter) =>
  ...
  const startMatching = async () => {
    ...
    try {
      ...
      if (analysisResult.value.chapters && analysisResult.value.chapters.length > 0) {
        ...
        console.log(
          `从${analysisResult.value.chapters.length}个章节中提取到${combinedEnvironmentTracks.length}个环境轨道`
        )
      ...
      // 调用环境音智能匹配API
      const response = await environmentGenerationAPI.matchEnvironmentSounds(matchingData, {
      ...
      if (response.data.enhanced_analysis_result) {
        ...
        matchingResult.value = {
          ...
          matching_summary:
            response.data.matching_summary ||
            response.data.enhanced_analysis_result.matching_summary,
    ...
    try {
      ...
      for (let i = 0; i < selectedPrompts.length; i++) {
        ...
        try {
          ...
          addGenerationLog(
            'success',
            `✅ ${prompt.title} 生成任务已启动 (ID: ${response.data.sound_id})`
          )
  ...
  const loadAllAnalyzedChapters = async () => {
    try {
      ...
      const allBooks =
        booksResponse.data.success && booksResponse.data.data
          ? booksResponse.data.data
          : booksResponse.data.data || booksResponse.data || []
      ...
      for (const book of allBooks) {
        try {
          ...
          const bookChapters =
            chaptersResponse.data.success && chaptersResponse.data.data
              ? chaptersResponse.data.data
              : chaptersResponse.data.data || chaptersResponse.data || []
