        ...
        <a-card title="声音克隆配置" :bordered="false" class="config-card">
          ...
          <a-card title="1. 上传参考音频" class="config-card">
            ...
            <div class="form-section">
              ...
              <p class="form-desc">
                上传清晰的音频文件（.wav格式，建议10-60秒），AI将学习其声音特征
              </p>
              ...
              <div v-if="audioFileInfo" class="file-info">
                <div class="file-item">
                  ...
                  <div class="file-details">
                    ...
                    <div class="file-meta">
                      {{ audioFileInfo.size }} | {{ audioFileInfo.duration }}
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="audio-result">
            <div class="audio-player">
              ...
              <div class="audio-player-unified">
                <a-button type="primary" size="large" block @click="playWithUnifiedPlayer">
                  <template #icon>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                      <path d="M8,5.14V19.14L19,12.14L8,5.14Z" />
                    </svg>
      ...
      if (synthesizeResponse.data.success && synthesizeResponse.data.audioUrl) {
        ...
        try {
          ...
          generatedAudio.value = {
            ...
            duration:
              audioCheck.duration && typeof audioCheck.duration === 'number'
                ? `${audioCheck.duration.toFixed(1)}秒`
                : '未知',
            quality: 4.0, // 默认质量评分
  ...
  const downloadAudio = async () => {
    ...
    try {
      ...
      try {
        ...
        link.href = isRelativePath
          ? generatedAudio.value.url
          : generatedAudio.value.fallbackUrl || generatedAudio.value.url
        link.download = `AI_Voice_${Date.now()}.wav`
        document.body.appendChild(link)
        link.click()
        document.body.removeChild(link)
