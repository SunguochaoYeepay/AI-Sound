  ...
  <div class="books-container">
      ...
      <a-spin :spinning="loading" tip="加载书籍中...">
        ...
        <div v-if="books.length === 0 && !loading" class="empty-state">
          <div class="empty-content">
            ...
            <h3>暂无书籍</h3>
            <p>创建您的第一本书籍</p>
            <a-button type="primary" @click="$router.push('/books/create')"> 立即创建 </a-button>
          </div>
        </div>
        ...
        <div v-else-if="viewMode === 'grid'" class="grid-view">
          <a-row :gutter="[16, 16]">
            <a-col v-for="book in books" :key="book.id" :xs="24" :sm="12" :md="8" :lg="6" :xl="6">
              <a-card hoverable class="book-card" @click="viewBookDetail(book.id)">
                <template #actions>
                  <EditOutlined key="edit" @click.stop="editBook(book.id)" title="编辑" />
                  <EyeOutlined key="view" @click.stop="viewBookDetail(book.id)" title="查看" />
                  <DeleteOutlined key="delete" @click.stop="deleteBook(book)" title="删除" />
                  <SoundOutlined
                    key="synthesis"
                  ...
                  <template #description>
                    <div class="book-info">
                      ...
                      <div class="book-tags" v-if="book.tags && book.tags.length > 0">
                        <a-tag v-for="tag in book.tags.slice(0, 3)" :key="tag" size="small">
                          {{ tag }}
                        </a-tag>
                      ...
                      <div class="book-dates">
                        <span class="date-item"> 创建: {{ formatDate(book.createdAt) }} </span>
                        <span class="date-item"> 更新: {{ formatDate(book.updatedAt) }} </span>
                      </div>
                    </div>
                  </template>
                </a-card-meta>
    ...
    >
      ...
      <p v-if="deleteModal.book?.synthesisProjects?.length > 0" style="color: red">
        ⚠️ 该书籍有
        {{ deleteModal.book.synthesisProjects.length }} 个关联的合成项目，删除后这些项目也会被删除。
      </p>
      <a-checkbox v-model:checked="deleteModal.force"> 强制删除（包括关联的合成项目） </a-checkbox>
    </a-modal>
  </div>
</template>
