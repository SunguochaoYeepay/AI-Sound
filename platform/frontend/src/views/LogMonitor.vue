<template>
  <div class="log-monitor">
    ...
    <div class="page-header">
      <a-page-header title="日志监控" sub-title="系统日志实时监控与分析" @back="$router.back()">
        <template #extra>
          <a-space>
            <a-button
              type="primary"
            ...
            <a-button :icon="h(DownloadOutlined)" @click="exportLogs"> 导出 </a-button>
            <a-button :icon="h(ClearOutlined)" @click="showClearModal = true"> 清理 </a-button>
          </a-space>
        </template>
      </a-page-header>
    </div>
    ...
    <a-row :gutter="[24, 24]" class="stats-row">
      <a-col :span="6">
        <a-card class="stat-card">
          <a-statistic title="总日志数" :value="stats.total" :loading="statsLoading">
            <template #prefix>
      ...
      <a-col :span="6">
        <a-card class="stat-card">
          <a-statistic title="错误率" :value="stats.error_rate" suffix="%" :loading="statsLoading">
            <template #prefix>
      ...
      <a-col :span="6">
        <a-card class="stat-card">
          <a-statistic title="最近错误" :value="stats.recent_errors" :loading="statsLoading">
            <template #prefix>
      ...
      <a-col :span="6">
        <a-card class="stat-card">
          <a-statistic title="自动刷新" :value="autoRefreshInterval" suffix="秒">
            <template #prefix>
      ...
      <a-form layout="inline" :model="filters" @finish="applyFilters">
        ...
        <a-form-item label="模块">
          <a-select
            ...
            allowClear
          >
            <a-select-option v-for="module in availableModules" :key="module" :value="module">
    ...
    <a-card class="log-table-card" title="日志列表">
      <template #extra>
        <a-space>
          ...
          <a-button size="small" type="link" @click="scrollToTop"> 回到顶部 </a-button>
        </a-space>
      </template>
      ...
      >
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.dataIndex === 'actions'">
            <a-space>
              <a-button type="link" size="small" @click="showLogDetail(record)"> 查看 </a-button>
              <a-button
    ...
    >
      ...
      <a-form-item label="保留天数">
        <a-input-number v-model:value="clearDays" :min="1" :max="365" style="width: 120px" />
...
<script setup>
  ...
  async function fetchMetadata() {
    try {
      const [levelsRes, modulesRes] = await Promise.all([logApi.getLevels(), logApi.getModules()])
