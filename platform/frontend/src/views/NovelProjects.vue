      ...
      <a-spin :spinning="loading" tip="加载项目中...">
        ...
        <div v-if="filteredProjects.length === 0 && !loading" class="empty-state">
          <div class="empty-content">
            ...
            <h3>暂无项目</h3>
            <p>创建您的第一个语音合成项目</p>
            <a-button type="primary" @click="goToCreatePage"> 立即创建 </a-button>
          </div>
        </div>
                ...
                <template v-if="hasAudioFiles(project)">
                  <a-dropdown @click.stop>
                    <a-button
                      ...
                      size="small"
                      :title="
                        isProjectCompleted(project) ? '选择重新合成方式' : '继续合成或重新合成'
                      "
                    >
                      {{ isProjectCompleted(project) ? '🔄 重新合成' : '⚡ 继续合成' }}
                      <DownOutlined />
                    </a-button>
                    <template #overlay>
            ...
            <template #bodyCell="{ column, record }">
              ...
              <template v-if="column.key === 'actions'">
                <div style="display: flex; gap: 8px">
                  ...
                  <!-- 已完成或部分完成项目：显示合成选项菜单 -->
                  <template
                    v-if="record.status === 'completed' || record.status === 'partial_completed'"
                  >
                    <a-dropdown @click.stop>
                      <a-button
                        type="primary"
                        size="small"
                        :title="
                          record.status === 'completed' ? '选择重新合成方式' : '继续合成或重新合成'
                        "
                      >
                        {{ record.status === 'completed' ? '🔄 重新合成' : '⚡ 继续合成' }}
                        <DownOutlined />
                      </a-button>
                      <template #overlay>
                  ...
                  <a-button
                    ...
                    size="small"
                    danger
                    @click.stop="confirmDeleteProject(record)"
                  >
                    删除
                  </a-button>
                </div>
    ...
    Modal.confirm({
      ...
      content: h('div', [
        ...
        h(
          ...
          {
            style:
              'margin-top: 16px; padding: 12px; background: #fff7e6; border: 1px solid #ffd591; border-radius: 6px;'
          },
          [
            h(
              'p',
              { style: 'margin: 0 0 8px 0; color: #fa8c16; font-weight: 500;' },
              '⚠️ 删除提示'
            ),
            h(
              'p',
              { style: 'margin: 0; font-size: 13px; color: #8c8c8c;' },
              '如果项目包含音频文件、文本段落等关联数据，可能需要强制删除'
            )
          ]
        ),
