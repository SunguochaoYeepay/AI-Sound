...
export function useWorkerManager(options = {}) {
  ...
  const handleWorkerMessage = (workerInstance, event) => {
    ...
    workerStats.value.averageProcessTime =
      (workerStats.value.averageProcessTime * (workerStats.value.completedTasks - 1) +
        processingTime) /
      workerStats.value.completedTasks
  ...
  const submitBatchTasks = async (tasks) => {
    const promises = tasks.map((task) => submitTask(task.type, task.data, task.options))
  ...
  return {
    ...
    generateWaveform: (channelData, options) =>
      submitTask('generate_waveform', { channelData, options }),
    calculatePeaks: (channelData, options) =>
      submitTask('calculate_peaks', { channelData, options }),
    normalizeAudio: (channelData, targetLevel) =>
      submitTask('normalize_audio', { channelData, targetLevel }),
