    ...
    <div v-show="!collapsed" class="assistant-content">
        ...
        <div v-if="features.projectImport.enabled" class="feature-controls">
          ...
          <div v-if="selectedProjectData" class="control-item">
            ...
            <div class="track-layout">
              <div class="layout-option">
                <a-radio-group v-model:value="features.projectImport.layoutMode" @change="onLayoutModeChange">
              ...
              <div v-if="features.projectImport.layoutMode === 'manual'" class="manual-layout-config">
                <div class="character-mapping" v-for="character in projectCharacters" :key="character">
                  <label>{{ character }}:</label>
                  <a-select 
                    v-model:value="features.projectImport.trackMapping[character]" 
                    style="width: 100px"
                  >
                    <a-select-option v-for="i in 10" :key="i" :value="i">轨道 {{ i }}</a-select-option>
                  </a-select>
                </div>
              </div>
            </div>
          ...
          <div v-if="importPreview.length > 0" class="analysis-results">
            ...
            <div class="import-list">
              <div
                v-for="(segment, index) in importPreview"
                :key="index"
                class="import-item"
              >
              ...
              <a-tab-pane key="json" tab="📄 JSON配置导入">
                ...
                <div v-if="environmentPreview.length > 0" class="control-item">
                  ...
                  <div class="environment-preview">
                    <div
                      v-for="(env, index) in environmentPreview"
                      :key="index"
                      class="env-item"
                    >
                      <div class="env-info">
                        <span class="env-id">{{ env.id }}</span>
                        <span class="env-time">{{ formatTime(env.start_time) }} - {{ formatTime(env.start_time + env.duration) }}</span>
                      </div>
                ...
                <div class="action-buttons">
                  <a-button @click="analyzeEnvironmentJson" :loading="analyzing" size="small" type="primary">
                    <template #icon><SearchOutlined /></template>
                    解析JSON
                  </a-button>
                  <a-button @click="importEnvironmentSounds" :disabled="!hasEnvironmentData" size="small">
              ...
              <a-tab-pane key="library" tab="🎵 环境音库选择">
                <div class="library-section">
                    ...
                    >
                      <div class="sound-info">
                        ...
                        <div class="sound-meta">
                          ...
                          <a-badge 
                            :status="sound.generation_status === 'completed' ? 'success' : 'processing'" 
                  ...
                  <div v-if="selectedEnvironmentSounds.length > 0" class="batch-import-actions">
                    ...
                    <div class="action-buttons">
                      <a-button @click="importSelectedEnvironments" :loading="environmentImporting" size="small" type="primary">
        ...
        <div v-if="features.emotionAnalysis.enabled" class="feature-controls">
          <div class="control-item">
            ...
            <div class="control-group">
              ...
              />
              <span class="value-display">{{ Math.round(features.emotionAnalysis.intensity * 100) }}%</span>
            </div>
          </div>
          ...
          <div class="action-buttons">
            ...
            <a-button @click="applyEmotionAdjustments" :disabled="!hasEmotionResults" size="small" type="primary">
              <template #icon><CheckOutlined /></template>
              应用调节
            </a-button>
          </div>
              ...
              <a-tab-pane key="recommend" tab="智能推荐">
                <div class="mode-content">
                  ...
                  <div class="control-item">
                    ...
                    <div class="control-group">
                      <a-slider
                        ...
                        :step="0.1"
                      />
                      <span class="value-display">{{ Math.round(features.musicRecommendation.intensity * 100) }}%</span>
                    </div>
                  </div>
                  ...
                  <div class="action-buttons">
                    ...
                    <a-button @click="previewMusic" :disabled="!hasMusicRecommendations" size="small">
                      <template #icon><PlayCircleOutlined /></template>
                      预览音乐
                    </a-button>
                  </div>
                      ...
                      >
                        <div class="music-info">
                          ...
                          <div class="music-meta" v-if="music.quality_rating">
                            <span class="quality-rating">★{{ music.quality_rating?.toFixed(1) || 'N/A' }}</span>
                ...
                <div class="mode-content">
                  ...
                  <div class="library-filters">
                    ...
                    <div style="display: flex; gap: 8px; align-items: center;">
                      ...
                      <a-button @click="loadMusicLibrary" :loading="musicLibraryLoading" size="small">
                        <template #icon><ReloadOutlined /></template>
                      </a-button>
                    </div>
                  </div>
                    ...
                    >
                      ...
                      <div class="sound-info">
                        ...
                        <div class="sound-meta">
                          ...
                          <span class="quality-rating">★{{ music.quality_rating?.toFixed(1) }}</span>
      ...
      <div class="feature-section">
        ...
        <div v-if="features.batchProcessing.enabled" class="feature-controls">
          ...
          <div class="action-buttons">
            <a-button @click="startBatchProcessing" :loading="batchProcessing" size="small" type="primary">
  ...
  try {
    ...
    if (response.data.success) {
      ...
      musicRecommendations.value = [
        { name: '宁静森林', description: '轻柔的自然环境音', style: 'ambient', quality_rating: 4.2 },
        { name: '温暖阳光', description: '温馨的背景音乐', style: 'ambient', quality_rating: 4.5 },
        { name: '梦幻空间', description: '空灵的电子音乐', style: 'electronic', quality_rating: 4.1 }
      ]
...
// 监听环境音导入模式变化，加载相应数据
watch(() => features.environmentImport.mode, (newMode) => {
...
// 监听音乐推荐模式变化，加载相应数据
watch(() => features.musicRecommendation.mode, (newMode) => {
...
.emotion-segment.neutral { background: #d9d9d9; }
.emotion-segment.happy { background: #52c41a; }
.emotion-segment.sad { background: #1890ff; }
.emotion-segment.excited { background: #fa8c16; }
.emotion-segment.angry { background: #ff4d4f; }
.emotion-segment.surprised { background: #722ed1; }
