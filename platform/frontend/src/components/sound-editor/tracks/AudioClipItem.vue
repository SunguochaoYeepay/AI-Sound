<template>
  ...
  >
    ...
    <div class="clip-content">
      ...
      <div v-if="clip.metadata?.character" class="clip-character">
        {{ clip.metadata.character }}
      </div>
    </div>
    ...
    <!-- 调整大小手柄 -->
    <div class="resize-handle resize-left" @mousedown.stop="startResize('left')"></div>
...
<script setup>
  ...
  import { getAudioWaveform } from '../../../api/sound-editor/audioFiles'
  ...
  function playClip() {
    try {
      // 构建音频文件URL
      const audioUrl = `/api/v1/sound-editor/audio-files/download/${props.clip.fileId}`
      ...
      audio.addEventListener('canplay', () => {
        audio.play()
        message.success(`播放音频片段: ${props.clip.filename}`)
      })

      audio.addEventListener('error', (e) => {
        console.error('音频播放失败:', e)
        message.error(`播放失败: ${props.clip.filename}`)
      })
  ...
  async function loadWaveformData() {
    if (!props.clip.fileId) return

    try {
      // fileId现在直接存储文件ID
      const fileId = props.clip.fileId

      if (fileId) {
        const result = await getAudioWaveform(
          fileId,
          Math.round(clipWidth.value),
          Math.round(clipHeight.value - 16)
        )
        if (result.success) {
