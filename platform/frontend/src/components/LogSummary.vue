<template>
  <div class="log-summary">
    <a-card title="系统日志概览" :hoverable="true">
      <template #extra>
        <a-space>
          <a-button type="link" size="small" @click="$router.push('/logs')"> 查看详情 </a-button>
          <a-button
            type="link"
            size="small"
      ...
      <a-row :gutter="[16, 16]" class="metrics-row">
        <a-col :span="8">
          <div class="metric-item">
            ...
            <div class="metric-trend" :class="{ 'trend-danger': stats.recent_errors > 10 }">
        ...
        <a-col :span="8">
          <div class="metric-item">
            ...
            <div class="metric-trend" :class="{ 'trend-danger': stats.error_rate > 5 }">
      ...
      <div class="level-distribution">
        <div v-for="(count, level) in stats.by_level" :key="level" class="level-item">
      ...
      <div class="module-activity">
        <a-row :gutter="[8, 8]">
          <a-col v-for="(count, module) in stats.by_module" :key="module" :span="6">
    ...
    <!-- 错误详情弹窗 -->
    <a-modal v-model:open="errorDetailVisible" title="错误详情" width="600px" :footer="null">
...
<script setup>
  ...
  async function refresh() {
    loading.value = true
    try {
      await Promise.all([fetchStats(), fetchRecentErrors()])
    } finally {
      loading.value = false
    }
  }
