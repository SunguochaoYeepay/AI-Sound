  ...
  <div class="audio-tester">
    ...
    <div class="tester-content">
      ...
      <div class="test-section">
        <h4>2. API音频测试</h4>
        <div class="audio-controls">
          <a-button @click="testApiAudio" :loading="isTestingApi"> 测试API音频 </a-button>
      ...
      <div class="test-section">
        ...
        <div class="audio-settings">
          <p>
            音量: <span class="volume-value">{{ volume }}%</span>
          </p>
  ...
  const playBuiltInSound = () => {
    ...
    try {
      ...
      testLogs.value.unshift(
        `[${new Date().toLocaleTimeString()}] 内置声音播放失败: ${error.message}`
      )
    }
  }
    ...
    try {
      ...
      try {
        ...
        if (relativeCheck.ok) {
          ...
          testLogs.value.unshift(
            `[${new Date().toLocaleTimeString()}] API音频加载成功: ${relativeUrl}`
          )
      ...
      try {
        ...
        if (absoluteCheck.ok) {
          ...
          testLogs.value.unshift(
            `[${new Date().toLocaleTimeString()}] API音频加载成功: ${absoluteUrl}`
          )
          apiResult.value = 'success'
          return
        }
      ...
      testLogs.value.unshift(
        `[${new Date().toLocaleTimeString()}] API音频测试失败: ${error.message}`
      )
  ...
  const testCustomUrl = async () => {
    ...
    try {
      ...
      audio.onloadeddata = () => {
        ...
        testLogs.value.unshift(
          `[${new Date().toLocaleTimeString()}] 自定义音频加载成功: ${(audio.duration || 0).toFixed(2)}秒`
        )
      }
      ...
      customResult.value = 'error'
      testLogs.value.unshift(
        `[${new Date().toLocaleTimeString()}] 自定义URL测试失败: ${error.message}`
      )
    }
  }
