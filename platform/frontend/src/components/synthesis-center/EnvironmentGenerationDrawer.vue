      ...
      <div v-if="editingTrack" class="track-edit-form">
        ...
        <a-form layout="vertical">
                      ...
                      >
                        <div class="sound-info">
                          ...
                          <div class="sound-meta">
                            <span class="sound-category">{{ sound.category?.name || '未分类' }}</span>
          ...
          <a-form-item label="音量设置">
            ...
            />
            <div class="form-help">
              当前音量：{{ (editForm.volume * 100).toFixed(0) }}%
            </div>
          </a-form-item>
...
const emit = defineEmits(['update:visible', 'complete', 'start-audio-mixing', 'environment-data-updated'])
...
const startAnalysis = async (forceReanalyze = false) => {
  try {
    ...
    // 显示预估时间提示
    const narrationCount = props.synthesisData?.synthesis_plan?.filter(seg => 
      seg.speaker === '旁白' || seg.character === '旁白'
    ).length || 0
    
    if (forceReanalyze) {
      message.info('正在重新分析环境音需求，请稍候...', 3)
    } else if (narrationCount > 10) {
      message.info(`检测到${narrationCount}个旁白段落，预计需要${Math.ceil(narrationCount * 10 / 60)}分钟分析时间，请耐心等待...`, 6)
    } else {
      message.info('正在分析环境音需求，请稍候...', 3)
    }
    
    const response = await api.analyzeEnvironment(
      props.projectId, 
      {
        ...props.synthesisData,
        options: {
...
const saveTrackEdits = async () => {
  try {
    ...
    const response = await api.updateTrackConfig(props.projectId, editingTrackIndex.value, manualEdits)
