<template>
  <div class="environment-config-manager">
          ...
          <template v-if="column.dataIndex === 'track_info'">
            <div class="track-info">
              ...
              <div class="track-time">
                {{ record.start_time?.toFixed(1) }}s - {{ (record.start_time + record.duration)?.toFixed(1) }}s
              </div>
            ...
            <div class="environment-id-config">
              <div class="id-display">
                <a-input
                  ...
                  size="small"
                  @blur="updateTrackConfig(index, { environment_id: record.environment_id, auto_generated_id: false })"
                />
              </div>
              <div class="id-status">
          ...
          <template v-else-if="column.dataIndex === 'actions'">
            <div class="track-actions">
              <a-button-group size="small">
                ...
                <a-button @click="editTrack(index)">
                  编辑
                </a-button>
                <a-button 
                  danger 
    ...
    <div v-else class="empty-state">
      ...
      >
        <a-button type="primary" @click="$emit('start-analysis')">
          开始环境音分析
        </a-button>
      </a-empty>
    </div>
      ...
      <div v-if="editingTrack" class="track-edit-form">
        <a-form layout="vertical">
          <a-form-item label="环境音ID">
            <a-input
              v-model:value="editingTrack.environment_id"
              placeholder="输入自定义环境音ID"
            />
            <div class="form-help">
              留空将自动生成ID，格式：env_段落ID_轨道序号
            </div>
          </a-form-item>
          ...
          <a-form-item>
            <a-checkbox v-model:checked="editingTrack.loop_enabled">
              启用循环播放
            </a-checkbox>
          </a-form-item>

          <div class="form-actions">
            <a-button @click="editDrawerVisible = false">
              取消
            </a-button>
            <a-button type="primary" @click="saveTrackEdit" :loading="saving">
              保存配置
            </a-button>
          </div>
        </a-form>
...
const hasConfig = computed(() => {
  return config.value && config.value.success && config.value.config?.environment_tracks?.length > 0
})

const keywordOptions = computed(() => {
  const commonKeywords = [
    '脚步声', '翻书声', '雷声', '雨声', '风声', '虫鸣', '鸟叫', '水声',
    '汽车声', '钟声', '门声', '窗声', '电话声', '键盘声', '音乐声'
  ]
...
const getSessionStatusColor = () => {
  ...
  switch (stage) {
    ...
    case 'validation_prepared': return '#1890ff'
