      ...
      <a-tabs v-model:activeKey="activeTab">
          ...
          <a-card title="处理信息" size="small" style="margin-bottom: 16px">
            <a-descriptions :column="1" bordered size="small">
              ...
              <a-descriptions-item label="生成片段">
                {{
                  processingInfo.total_segments ||
                  analysisData.synthesis_json?.synthesis_plan?.length ||
                  0
                }}
                个
              </a-descriptions-item>
              <a-descriptions-item label="检测角色">
                {{
                  processingInfo.characters_found ||
                  analysisData.synthesis_json?.characters?.length ||
                  0
                }}
                个
              </a-descriptions-item>
                ...
                <div class="voice-assignment">
                  ...
                  <div v-else class="voice-not-in-library">
                    <div class="voice-selection-area">
                        ...
                        >
                          ...
                          >
                            <div class="character-option">
                              ...
                              <a-tag v-if="libChar.is_voice_configured" color="green" size="small"
                                >已配音</a-tag
                              >
                      ...
                      <!-- 显示选择的角色信息 -->
                      <div
                        v-if="character.selected_character_info"
                        class="selected-character-info"
                      ...
                      >
                        ...
                        <a-tag
                          v-if="character.selected_character_info.is_voice_configured"
                          color="green"
                        >
                          ✓ 有音频配置
                        </a-tag>
        ...
        <a-tab-pane tab="🔧 JSON数据" key="json">
          <div class="json-editor">
            <a-textarea :value="getJsonPreview()" :rows="25" readonly class="json-display" />
          </div>
        </a-tab-pane>
      </a-tabs>
    </div>
...
<script setup>
  ...
  const initEditableData = () => {
    ...
    console.log(
      '[EditableAnalysisDrawer] characters详细内容:',
      JSON.stringify(synthesisJson.characters, null, 2)
    )
    console.log(
      '[EditableAnalysisDrawer] synthesis_plan详细内容:',
      JSON.stringify(synthesisJson.synthesis_plan, null, 2)
    )
    ...
    if (synthesisJson.characters && synthesisJson.characters.length > 0) {
      ...
      // 统计所有说话人及其出现次数
      console.log(
        '[EditableAnalysisDrawer] 所有原始speaker值:',
        ...
        if (!speakerMap.has(speaker)) {
          speakerMap.set(speaker, {
            ...
            voice_type:
              speaker === '旁白'
                ? 'narrator'
                : speaker.includes('孙悟空')
                  ? 'male'
                  : speaker.includes('唐僧')
                    ? 'male'
                    : speaker.includes('猪八戒')
                      ? 'male'
                      : speaker.includes('沙僧')
                        ? 'male'
                        : speaker.includes('白骨精')
                          ? 'female'
                          : 'neutral',
            count: 0
          })
    ...
    console.log(
      '[EditableAnalysisDrawer] 最终角色列表:',
      JSON.stringify(editableCharacters.value, null, 2)
    )
    ...
    try {
      ...
      if (response.data?.success && response.data.data?.length > 0) {
        ...
        editableCharacters.value.forEach((character) => {
          const matchedCharacter = characterLibrary.find(
            (libChar) =>
  ...
  const getDialogueRatio = () => {
    ...
    const dialogueCount = editableSegments.value.filter(
      (segment) => segment.speaker && segment.speaker !== '旁白' && segment.speaker !== '系统旁白'
    ).length
