  ...
  <div class="audio-enhancement">
    <a-card title="🎵 音频增强设置" size="small">
      ...
      <a-form-item label="启用音效增强">
        <a-switch v-model:checked="enhancementEnabled" @change="onEnhancementToggle" />
      ...
      <div v-if="enhancementEnabled" class="enhancement-settings">
        ...
        <a-form-item>
          <a-space>
            <a-button type="primary" @click="testAudioGeneration" :loading="testing">
              🎧 测试音效生成
            </a-button>
    ...
    <div v-if="previewAudio" class="audio-preview">
      <a-card title="🎵 音效预览" size="small">
        <audio controls :src="previewAudio" style="width: 100%">您的浏览器不支持音频播放</audio>
      </a-card>
    </div>
  </div>
</template>
...
<script setup>
  ...
  const presetScenes = ref([
    ...
    {
      name: '🏰 古堡',
      prompt: 'medieval castle ambience with wind',
      volumes: { voice: 80, bg: 35 }
    },
