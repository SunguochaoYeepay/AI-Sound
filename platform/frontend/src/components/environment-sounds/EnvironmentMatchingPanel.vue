  ...
  <div class="environment-matching-panel">
    <div class="panel-header">
      ...
      <div class="header-actions">
        ...
        <a-button @click="clearResults" :disabled="!hasResults" size="small">
          清除结果
        </a-button>
      </div>
    </div>
    ...
    <div v-if="matchingSummary" class="matching-summary">
      <a-row :gutter="16">
        ...
        <a-col :span="6">
          <a-statistic title="已匹配" :value="matchingSummary.matched_tracks" :value-style="{ color: '#52c41a' }" />
        </a-col>
        <a-col :span="6">
          <a-statistic title="需生成" :value="matchingSummary.need_generation_tracks" :value-style="{ color: '#fa8c16' }" />
        </a-col>
        <a-col :span="6">
          <a-statistic title="匹配率" :value="matchingSummary.match_rate" suffix="%" :value-style="getMatchRateStyle()" />
        </a-col>
      </a-row>
    </div>
    ...
    <div v-if="hasResults" class="matching-results">
            ...
            <a-list-item>
              <a-list-item-meta>
                <template #title>
                  ...
                  <a-tag color="green" size="small">匹配度: {{ (item.confidence * 100).toFixed(0) }}%</a-tag>
                </template>
              ...
              <template #actions>
                ...
                <a-button type="link" size="small" @click="confirmMatch(item)">
                  确认
                </a-button>
              </template>
            </a-list-item>
      ...
      <div class="generation-section">
        ...
        <a-list 
          :data-source="needGenerationSounds" 
          size="small"
          :pagination="false"
        >
          <template #renderItem="{ item }">
      ...
      <div class="panel-actions">
        ...
        <a-button @click="exportResults" size="large">
          导出配置
        </a-button>
      </div>
    </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const canProceed = computed(() => {
      return hasResults.value && (matchedSounds.value.length > 0 || needGenerationSounds.value.length > 0)
    })
    ...
    const startMatching = async () => {
      ...
      if (props.analysisResult) {
        ...
        if (props.analysisResult.environment_tracks && props.analysisResult.environment_tracks.length > 0) {
          hasEnvironmentTracks = true
        }
        // 或者包含chapters格式
        else if (props.analysisResult.chapters && props.analysisResult.chapters.length > 0) {
          const hasChapterTracks = props.analysisResult.chapters.some(chapter => 
