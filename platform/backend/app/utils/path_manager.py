...
class PathManager:
    ...
    def __init__(self):
        ...
        logger.debug(f"路径管理器初始化: Docker={self.is_docker}, Windows={self.is_windows}")
        logger.debug(f"基础路径: {self.base_paths}")
    ...
    def _find_project_root(self) -> Path:
        ...
        for parent in [current] + list(current.parents):
            if any((parent / marker).exists() for marker in markers):
                logger.debug(f"找到项目根目录: {parent}")
                return parent
        
        # 如果找不到，使用当前目录
        logger.debug(f"未找到项目根目录，使用当前目录: {current}")
        return current
