...
from app.services.llm_scene_analyzer import llm_scene_analyzer, LLMSceneAnalysisResult
    ...
    try:
        if request.use_llm:
            ...
            response = AnalysisResultResponse(
                text_hash=llm_result.text_hash,
                ...
                unique_atmospheres=list(set(s.atmosphere for s in llm_result.analyzed_scenes)),
                recommended_durations=[p.get("duration", 15.0) for p in enhanced_prompts],
                # LLM特有字段
                narrative_analysis=llm_result.narrative_analysis,
                emotional_progression=llm_result.emotional_progression,
                scene_transitions=llm_result.scene_transitions,
                recommended_soundscape=llm_result.recommended_soundscape,
