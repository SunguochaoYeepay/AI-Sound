...
async def log_system_event(
        ...
        try:
                ...
                if not db or not hasattr(db, 'add'):
                        logger.warning("数据库会话无效，跳过系统日志记录")
                        return
                ...
                # 添加到会话（不自动commit/rollback避免事务冲突）
                # 由调用者负责最终提交或回滚
                db.add(log_entry)
...
async def update_usage_stats(
        ...
        try:
                ...
                if not db or not hasattr(db, 'query'):
                        logger.warning("数据库会话无效，跳过使用统计更新")
                        return
...
def validate_audio_file(file_path: str) -> Dict[str, Any]:
        ...
        try:
                ...
                if file_ext not in supported_formats:
                        return {"valid": False, "error": f"不支持的音频类型: {file_ext}"}
                ...
                return {"valid": False, "error": f"检查失败: {str(e)}"}
...
def get_audio_duration(file_path: str) -> Optional[float]:
        ...
        try:
                ...
                if not hasattr(get_audio_duration, '_warning_shown'):
                        logger.warning("无法获取音频时长-缺少 librosa 或 pydub")
...
def validate_parameters(
        ...
        if not isinstance(p_weight, (int, float)) or p_weight < 0.1 or p_weight > 3.0:
                errors.append("p_weight必须是0.1-3.0之间的浮点数")
        
        # 检查t_weight
        if not isinstance(t_weight, (int, float)) or t_weight < 0.1 or t_weight > 3.0:
                errors.append("t_weight必须是0.1-3.0之间的浮点数")
