...
def check_project_88_tracks():
    """检查项目88的轨道信息"""
    
    # 检查项目88的详情
    project_url = "http://localhost:8001/api/v1/environment-generation/projects/88"
    
    try:
        response = requests.get(project_url)
        print(f"项目88详情API状态码: {response.status_code}")
        ...
        if response.status_code == 200:
            ...
            print(f"❌ 项目88详情API失败: {response.text}")
            
    except Exception as e:
        print(f"❌ 检查项目88轨道信息失败: {e}")

if __name__ == "__main__":
    print("=== 检查项目88的轨道信息 ===")
    check_project_88_tracks()